...
extern S16 cal_days(DATE a,DATE b)
{
        ...
        if((i = is_late(a,b)) < 0) {                                                /**<date a is  early than date b */
                ptr = ptr_to_year[(is_leap_year(a->year))];
                sum += ptr[a->month];                              /**< calculate the rest days of year a  */
                for(j = a->year +1; j <= b->year; j++) {
                        if(is_leap_year(j))                        /**< calculate rest years */
                                sum += 366;
                        else
                                sum += 365;
                }
                ptr = ptr_to_year[(is_leap_year(b->year))];
                sum -= ptr[b->month];                              /**< calculate the days last in year b */
        } else {
